...
import { User, CourseService } from '../../shared';
        ...
        return new Promise<NgbModalRef>((resolve, reject) => {
            ...
            if (id) {
                this.courseService.find(id).subscribe((response) => {
                    this.ngbModalRef = this.courseModalRef(component, response.body);
                    resolve(this.ngbModalRef);
                });
            } else {
                // setTimeout used as a workaround for getting ExpressionChangedAfterItHasBeenCheckedError
                setTimeout(() => {
                    this.ngbModalRef = this.courseModalRef(component, new Course(null, '', []));
                    resolve(this.ngbModalRef);
                }, 0);
            }
        });
